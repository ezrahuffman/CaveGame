...
and the systems (inventory, equipment, lighting, combat, etc...). To generate the caves for the map I first create a graph of nodes 
 ...
 After the path is constructed I use marching squares to render the path. This is used to create meshes (for each chunk of the map) and to
  ...
  \
I have also added subdivision and chunking to make the path smoother and allow for faster load times. Having one large mesh for the map meant that loading the scene took minutes to load sometimes. By chunking the mesh into blocks, as I had already done for the tiled parts of the map, I was able to cut the load times to seconds.

It is unlikely I return to make this into a fully fleshed out game, but you never know. 
